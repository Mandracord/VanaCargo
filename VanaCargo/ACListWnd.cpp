...
CACListWnd::CACListWnd()
{
	...
	m_bSelecting = false;
	m_pBoldFontDC = m_pFontDC = NULL;
	m_hBoldFont = m_hFont = NULL;
}
...
void CACListWnd::DrawItem(CDC* pDC, long m_lItem, long width)
{
	...
	{
		CString SearchStr, EditText, Part0, Part1, Part2;
		int StrPos, StrLen, DisplayStrLen;
		CSize TextExtent;
		CRect DrawRect;
		CFont *pFont;
		...
		SearchStr.MakeLower();
		DisplayStrLen = SearchStr.GetLength();
		// retrieve the text of the edit
		m_pEditParent->GetWindowText(EditText);
		EditText.MakeLower();
		// split the string into 3
		StrLen = EditText.GetLength();
		StrPos = SearchStr.Find(EditText);

		Part0 = m_DisplayStr.Left(StrPos);
		Part1 = m_DisplayStr.Mid(StrPos, StrLen);
		Part2 = m_DisplayStr.Right(DisplayStrLen - StrPos - StrLen);

		TextExtent = pDC->GetTextExtent(Part0);
		DrawRect.left = 2;
		DrawRect.right = DrawRect.left + TextExtent.cx;

		pDC->DrawText(Part0, -1, DrawRect, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER);
		pFont = pDC->SelectObject(m_pBoldFontDC);

		TextExtent = pDC->GetTextExtent(Part1);
		DrawRect.left = DrawRect.right;
		DrawRect.right = DrawRect.left + TextExtent.cx;

		pDC->DrawText(Part1, -1, DrawRect, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER);
		pDC->SelectObject(pFont);

		if (Part2.IsEmpty() == false)
		{
			TextExtent = pDC->GetTextExtent(Part2);
			DrawRect.left = DrawRect.right;
			DrawRect.right = width;

			pDC->SetTextColor(::GetSysColor(COLOR_WINDOWTEXT));
			pDC->DrawText(Part2, -1, DrawRect, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER | DT_END_ELLIPSIS);
		}
	}
	else
	{
...
int CACListWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_hFont = GetDefaultFont(LogFont);
	m_pFontDC = CFont::FromHandle(m_hFont);

	LogFont.lfWeight = FW_BOLD;
	m_hBoldFont = CreateFontIndirect(&LogFont);
	m_pBoldFontDC = CFont::FromHandle(m_hBoldFont);

	return Result;
}

void CACListWnd::OnDestroy()
{
	if (m_hBoldFont != NULL)
	{
...
HFONT CACListWnd::GetDefaultFont(LOGFONT &LogFont)
{
	GetObject(GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &LogFont);

	if (IsWindowsVersionOrGreater(5, 0, 0))
	{
		if (_tcsicmp(LogFont.lfFaceName, _T("MS Shell Dlg")))
			_tcscat_s(LogFont.lfFaceName, _T(" 2"));
	}
